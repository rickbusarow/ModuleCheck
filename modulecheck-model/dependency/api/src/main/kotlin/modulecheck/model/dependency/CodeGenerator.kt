...
interface CodeGenerator {
  ...
  /**
   * A human-readable, descriptive name for this specific compiler
   * extension, such as 'Hilt Android' or 'Moshi Kotlin codegen (KSP)'.
   *
  ...
  /**
   * This is the .jar for the compiler library, not the Gradle plugin. For
   * instance, `androidx.room:room-compiler` or `com.squareup.anvil:compiler`.
   *
   * For KSP, annotation processors, and Anvil, the extensions are typically added in the
   * build file like `kapt("com.example.foo:compiler:1.2.3")`. For a Kotlin compiler plugin,
   * this will probably be an "invisible" dependency added via the library's Gradle plugin.
   *
   * @since 0.12.0
   */
  ...
  /**
   * The *fully qualified* names of all annotations which make this compiler
   * extension do something -- typically code generation or perhaps static analysis.
   *
   * For instance, `androidx.room.Database` triggers compilation
   * for the Room compiler, so it would be included in this list.
   *
   * The annotations don't necessarily need to come from the same library, so
   * long as the compiler extension looks for them. For instance, `javax.Inject`
   * is included in the list of annotations for both Anvil and Dagger.
   *
   * On the other hand, an annotation from the library does not need to be
   * listed just because it's an annotation. An example of this would be
   * `tangle.inject.InternalTangleApi`, which is not used by the compiler.
   *
   * @since 0.12.0
   */
 ...
 * [ConfiguredDependency][modulecheck.parsing.gradle.model.ConfiguredDependency])
 * is associated with an established [CodeGenerator].
 *
 * @since 0.12.0
 */
