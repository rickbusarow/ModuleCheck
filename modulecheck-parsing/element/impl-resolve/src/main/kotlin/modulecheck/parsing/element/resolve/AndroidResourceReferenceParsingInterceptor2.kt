...
class AndroidResourceReferenceParsingInterceptor2(
  ...
  private suspend fun LazySet<AndroidRDeclaredName>.mapResolved(
    packet: NameParser.NameParserPacket,
    ...
    return mapNotNull { declaredName ->
      ...
      when {
        name in packet.imports -> AndroidRReferenceName(
          declaredName.packageName,
          packet.referenceLanguage
        )

        name in resolvedStrings -> AndroidRReferenceName(
          declaredName.packageName,
          packet.referenceLanguage
        )

        name in packet.unresolved -> AndroidRReferenceName(
          declaredName.packageName,
          packet.referenceLanguage
        )

        resolvedStrings
          .any { it.startsWith(name) } -> AndroidRReferenceName(
          declaredName.packageName,
          packet.referenceLanguage
        )
        ...
        (localROrNull == null || localROrNull == declaredName) && packet.wildcardImports
          .any { wildcard ->
            wildcard.replace('*', 'R')
              .startsWith(name)
          } -> AndroidRReferenceName(declaredName.packageName, packet.referenceLanguage)
  ...
  private suspend fun NameParser.NameParserPacket.findLocalROrNull(
