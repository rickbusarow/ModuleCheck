...
:::note ModuleCheck leverages Gradle for parsing all build logic, so validation works regardless of
whether that logic is expressed in Groovy or Kotlin.

However, auto-correction is only supported for Kotlin build files.
:::
