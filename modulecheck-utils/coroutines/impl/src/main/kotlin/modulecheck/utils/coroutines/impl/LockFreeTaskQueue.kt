 ...
 *
 * **Note 1: This queue is NOT linearizable. It provides only quiescent
 * consistency for its operations.** However, this guarantee is strong enough
 * for task-scheduling purposes. In particular, the following execution is
 * permitted for this queue, but is not permitted for a linearizable queue:
 *
 * ```
 ...
 * ```
 *
 * **Note 2: When this queue is used with multiple consumers (`singleConsumer == false`) this
 * it is NOT lock-free.** In particular, consumer spins until producer finishes its operation in
 * the case of near-empty queue. It is a very short window that could manifest itself rarely and
 * only under specific load conditions, but it still deprives this algorithm of its lock-freedom.
 *
 * @since 0.12.0
 */
internal open class LockFreeTaskQueue<E : Any>(
  ...
  val size: Int get() = _cur.get().size
  ...
  fun addLast(element: E): Boolean {
    _cur.loop { cur ->
      when (cur.addLast(element)) {
        LockFreeTaskQueueCore.ADD_SUCCESS -> return true
        LockFreeTaskQueueCore.ADD_CLOSED -> return false
        LockFreeTaskQueueCore.ADD_FROZEN -> _cur.compareAndSet(cur, cur.next()) // move to next
      }
    }
  }
  ...
  fun removeFirstOrNull(): E? {
    _cur.loop { cur ->
      ...
      if (result !== LockFreeTaskQueueCore.REMOVE_FROZEN) return result as E?
      _cur.compareAndSet(cur, cur.next())
    }
  }

  // Used for validation in tests only
  fun <R> map(transform: (E) -> R): List<R> = _cur.get().map(transform)

  // Used for validation in tests only
  fun isClosed(): Boolean = _cur.get().isClosed()
}
...
internal class LockFreeTaskQueueCore<E : Any>(
  ...
  private val _state = AtomicReference(0L)
  ...
  val size: Int get() = _state.get().withState { head, tail -> (tail - head) and MAX_CAPACITY_MASK }
  ...
  fun removeFirstOrNull(): Any? {
    _state.loop { state ->
      ...
      state.withState { head, tail ->
        ...
        // we cannot put null into array here, because copying thread could replace it with
        // Placeholder and that is a disaster
  ...
  private fun markFrozen(): Long {
    return _state.updateAndGet { state ->
      if (state and FROZEN_MASK != 0L) {
  ...
  private fun allocateNextCopy(state: Long): Core<E> {
    ...
    state.withState { head, tail ->
      ...
      while (index and mask != tail and mask) {
        ...
        val value = array[index and mask] ?: Placeholder(index)
        next.array[index and next.mask] = value
        index++
      }
  ...
  fun <R> map(transform: (E) -> R): List<R> {
    ...
    _state.get().withState { head, tail ->
      var index = head
      while (index and mask != tail and mask) {
        // replace nulls with placeholders on copy
  ...
  // Used for validation in tests only
  fun isClosed(): Boolean = _state.get() and CLOSED_MASK != 0L
